...
export function NodeGraph() {
  ...
  const why = state.selectedEdgeId
    ? edgeWhy(state, state.selectedEdgeId)
    : state.selectedNodeId
      ? nodeWhy(state, state.selectedNodeId)
      : null;
        ...
        <p className="text-[11px] leading-snug text-muted">
          {arming
            ...
            : why
              ? why
              : clustered
                ? "Uzak bakış: kurum kümeleri. Yakın’a in."
                : visibleNodes.length > 12
                  ? "Odak: ısınan ve seçili komşular. Diğerleri soluk."
                  : "Kutu = kurum · daire = kişi · baklava = koridor. Çizgiye bas: bağ."}
        </p>
      </div>
    </div>
  );
